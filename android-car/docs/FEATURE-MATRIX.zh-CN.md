...
| **CAR-STAGE-MAX** | 上游默认测试 / emily 拉满 | **已交付**（真车待验收） | P3-视觉 | **舞台最大化 `ab08c20`**：`setPreset(0)`、`setRenderQuality('ultra')`、`setShelfMode('stage')` + `setShelfPresence('always')`、`toggleFx` 粒子/电影/溢光等、滑条预算（coverRes **1.55** 等）。非仅 CSS 压暗。 |
...
| **CAR-AUDIO-FOCUS** | 系统音频会话 / 后台 | **待验收** | P4-媒体 | 音频焦点、蓝牙媒体通道、熄屏/ACC/休眠恢复、后台播放均未宣称通过。 |

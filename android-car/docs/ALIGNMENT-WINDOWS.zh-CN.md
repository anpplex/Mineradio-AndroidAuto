...
| 分支 | `huawei-android12-car` 为集成分支；`codex/*` 为任务分支；仅 push `origin`（B0） |
...
| 网易云/QQ 等登录 | 入口壳 + 插件 | **部分** | 全局车机账号入口打开原生登录 modal；插件管理（空首页 FAB） |
...
| **Wallpaper Engine / 全桌面** | 默认/生产主路径禁止 | **非目标** | 用户授权沙盒独立插件不计入 Windows 功能对齐 |
...
- Wallpaper Engine（默认/生产功能对齐非目标；独立沙盒插件见 `WALLPAPER-PLUGIN-DEVELOPMENT.zh-CN.md`）
- 完整桌面模式 / 桌面图标层
- 桌面歌词独立窗
- Electron 托盘与单实例
...
| 账号 | 用户胶囊 | 车机化右上角账号入口；播放页可达，原始用户胶囊/Apex 隐藏 |
...
| 优先级 | 项 | 说明 |
| --- | --- | --- |
| P1 | 真车验收播放/登录/SPICa/AF | 不宣称全兼容；当前只完成入口与车机热区适配 |
...
| NG | WE / 桌面歌词 | 默认/生产对齐不做；WE 沙盒独立跟踪 |

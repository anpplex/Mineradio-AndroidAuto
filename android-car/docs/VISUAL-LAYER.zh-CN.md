...
| 区 | 控件 | 尺寸 | 边距 | 备注 |
| --- | --- | --- | --- | --- |
| **左上 TL** | `#playlist-toggle` + `#home-btn` | **72×72** ×2 | 顶 = **safe-top+8**；左 **20**；间距 **12** | Home 从右上迁入 |
| **左下 BL** | `#fx-fab` 视觉控制台 | **72×72** | 左 **20**；底 = **safe-bottom + 底栏 + 12** | 从右下迁入 |
| **右上 TR** | （播放空）空首页 `#car-login-entry` | 登录高 **72** | 顶/右 clear safe | 无 APEX 常驻 |
| **~~行车/巡航/舞台~~** | **已移除常驻 UI** | — | — | 车机固定 **stage** showcase；`setMode` 仅 API |

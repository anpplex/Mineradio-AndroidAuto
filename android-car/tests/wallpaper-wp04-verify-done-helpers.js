...
/**
 * Helpers for WP-04 verify-done / CLOSE-VERIFY RED-01 contracts.
 *
 ...
 *
 * RED-01: production evaluate_wp04_verify_done is absent — GREEN implements it.
 * Unavailable signature must be WP04_VERIFY_DONE_UNAVAILABLE (not WP03_* mis-tag).
 */
...
const TASK_ID = 'WP-04';
const EXPECTED_WEIGHT = 10;
/** WP-00(4)+WP-01(6)+WP-02(8)+WP-03(8)=26; +WP-04(10)=36 when EffectiveDone. */
const EXPECTED_PROGRESS_WHEN_DONE = 36;
const EXPECTED_PROGRESS_PRE_DONE = 26;
const IMPLEMENTATION_PR = 12;
const IMPLEMENTATION_HEAD =
  'a2031695c6c8f23a1037dda3811030136825d09d';
const IMPLEMENTATION_MERGE =
  'eb1b6dca8e47500afc6c95d4fd2e6110bb0b54f0';
const IMPLEMENTATION_HEAD_REF = 'codex/wallpaper-plugin-wp04';
...
const FailureReason = Object.freeze({
  ...
  WP04_VERIFY_DONE_PROOF_MISSING: 'WP04_VERIFY_DONE_PROOF_MISSING',
  WP04_VERIFY_DONE_CALLER_FORGERY: 'WP04_VERIFY_DONE_CALLER_FORGERY',
  WP04_PREREQUISITE_NOT_DONE: 'WP04_PREREQUISITE_NOT_DONE',
  WP04_CATALOG_ENTRY_MISSING: 'WP04_CATALOG_ENTRY_MISSING',
  WP04_CATALOG_PROOF_INVALID: 'WP04_CATALOG_PROOF_INVALID',
  WP04_SUITE_RECEIPT_INVALID: 'WP04_SUITE_RECEIPT_INVALID',
  WP04_PR_PROOF_INVALID: 'WP04_PR_PROOF_INVALID',
...
/** GREEN must emit WP04_VERIFY_DONE_UNAVAILABLE when path missing (not WP03_*). */
const UNAVAILABLE_RE = /WP04_VERIFY_DONE_UNAVAILABLE/;
const MISTAG_RE = /WP03_VERIFY_DONE_UNAVAILABLE/;
...
function loadWp04CatalogTask(catalogFile = catalogPath) {
  if (!pathExists(catalogFile)) {
    return { ok: false, failureReason: FailureReason.WP04_CATALOG_ENTRY_MISSING };
  }
...
function assertWp04ProductionSurfacesPresent() {
  const missing = [];
  for (const p of [contractPath, patcherPath, smaliBridgePath, bridgeUnitTestPath]) {
    if (!pathExists(p)) missing.push(p);
  }
  return {
    ok: missing.length === 0,
    missing,
    failureReason: missing.length ? 'WP04_PRODUCTION_SURFACE_MISSING' : null,
  };
}

function tempReceipt(taskId = TASK_ID) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'wp04-vdone-'));
...
/**
 * Dynamic PR #12 identity from gh API (not caller merged/REMOTE_VERIFIED).
 */
...
function suiteProofs(options = {}) {
  return {
    ...
    wp04ContractTest: options.wp04ContractTest || {
      pass: true,
      sha256: 'c'.repeat(64),
    },
    fullNodeTest: options.fullNodeTest || { pass: true, sha256: 'd'.repeat(64) },
...
/** Baseline through WP-03 only (26%). */
...
/** Include live WP-04 receipt (elevates only when EffectiveDone truly true). */
...
/**
 * Live operational WP-04 progress from transaction receipt.
 * Through WP-03 = 26%; +WP-04(10)=36 when EffectiveDone.
 */
...
/**
 * True only when production has an explicit WP-04 evaluate path + WP04_* namespace.
 * Presence of WP03_VERIFY_DONE_UNAVAILABLE mis-tag alone is NOT sufficient.
 */
function productionHasWp04VerifyDonePath() {
  const src = fs.readFileSync(runnerPath, 'utf8');
  const hasEval = /evaluate_wp04_verify_done/.test(src);
  const hasDispatch =
    /elif task_id == ["']WP-04["']/.test(src) ||
    /task_id == ["']WP-04["']/.test(src);
  const hasNamespace =
    /WP04_VERIFY_DONE_UNAVAILABLE/.test(src) &&
    /WP04_VERIFY_DONE_PROOF_MISSING/.test(src) &&
    /WP04_VERIFY_DONE_CALLER_FORGERY/.test(src);
